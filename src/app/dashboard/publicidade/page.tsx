  ...
  const resultados = await Promise.all(
    (contas ?? []).map(async (conta) => {
      try {
        ...
        // Tag de siteId + advertiserId por campanha -- siteId e necessario
        // para a chamada de detalhe das metricas avancadas, e advertiserId
        // para montar o link da pagina de detalhe da campanha.
        const campanhasPeriodo = listasPeriodo.flatMap((r, i) =>
          r.campanhas.map((c) => ({ ...c, siteId: anunciantes[i].siteId, advertiserId: anunciantes[i].advertiserId }))
        );
    ...
    .map((r) => {
      ...
      const campanhas: CampanhaFormatada[] = r.campanhasPeriodo.map((c) => {
        ...
        const paramsDetalhe = new URLSearchParams({
          site: c.siteId,
          advertiser: String(c.advertiserId),
          periodo: presetSelecionado,
          de,
          ate,
        });
        return {
          ...
          hrefDetalhe: `/dashboard/publicidade/campanha/${r.conta.id}/${c.id}?${paramsDetalhe.toString()}`,
        };
      });
