...
export type CampanhaFormatada = {
  ...
  // Link pronto (ja com site/advertiser/periodo) para a pagina de detalhe
  // da campanha -- montado no servidor porque so la temos siteId e
  // advertiserId de cada anunciante (ver page.tsx).
  hrefDetalhe: string;
};
...
// Card agora e um link real para a pagina de detalhe da campanha (com todos
// os anuncios dela, nao so os que aparecem no top-10 da conta) -- por ser um
// <Link>, ctrl/cmd+click ou clique do meio ja abre em nova aba de graca.
function CampanhaCard({ c }: { c: CampanhaFormatada }) {
  ...
  return (
    <Link
      href={c.hrefDetalhe}
      className="block rounded-lg border border-gray-200 bg-white p-3 transition-colors hover:border-[var(--color-sixxis-blue)] hover:shadow-sm dark:border-gray-700 dark:bg-gray-800"
    >
      <div className="flex items-start justify-between gap-2">
        <p className="text-sm font-semibold leading-tight text-gray-800 dark:text-gray-100">
          {c.nome}
          <span className="ml-1 align-middle text-[10px] font-normal text-[var(--color-sixxis-blue)]">
            Ver anúncios →
          </span>
        </p>
...
// Ranking de anuncios agora e retratil (pedido do usuario) -- comeca
// fechado pra nao ocupar tanto espaco quando a conta tem muitos anuncios.
function RankingAnuncios({ anuncios }: { anuncios: AnuncioFormatado[] }) {
  const [aberto, setAberto] = useState(false);
  return (
    <div className="mt-4">
      <button
        onClick={() => setAberto((v) => !v)}
        className="mb-1 flex w-full items-center justify-between text-left text-xs font-semibold text-gray-500 hover:text-gray-700 dark:text-gray-400 dark:hover:text-gray-200"
      >
        <span>Ranking de anúncios · dado real ({anuncios.length})</span>
        <svg
          className={`h-3.5 w-3.5 shrink-0 text-gray-400 transition-transform ${aberto ? "rotate-180" : ""}`}
          viewBox="0 0 20 20"
          fill="currentColor"
        >
          <path
        ...
        <div className="border-t border-gray-200 bg-gray-50 p-4 dark:border-gray-700 dark:bg-gray-900/30">
          {conta.erro ? (
            ...
            <>
              ...
              {conta.anuncios.length > 0 && <RankingAnuncios anuncios={conta.anuncios} />}
