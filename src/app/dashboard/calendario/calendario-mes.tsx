...
import { CATEGORIAS_TAREFA, hexDaCor } from "@/lib/google/cores-evento";
...
function corDoEvento(evento: EventoCalendario): string {
  return hexDaCor(evento.cor);
}
...
type Props = {
    ...
    podeEditar: boolean;
  contas: { id: string; nome: string; colorId: string }[];
};

export default function CalendarioMes({ eventosIniciais, conectadoGoogle, podeEditar, contas }: Props) {
    const hoje = new Date();
...
async function handleCancelarEvento(eventoId: string) {
