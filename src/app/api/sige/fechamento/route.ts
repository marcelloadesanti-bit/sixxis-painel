...
// 2) busca os numeros "ao vivo" de ML + Amazon para o mesmo periodo;
// 3) congela tudo (sige_fechamentos + sige_fechamento_itens +
//    sige_fechamento_ads_itens) -- e esse congelamento que alimenta o
...
export async function POST(request: Request) {
  ...
  // (e quaisquer outros ja lancados que se sobreponham ao periodo).
  const [itensAuto, itensManuaisTodos] = await Promise.all([
  ...
  if (adsManuais.length > 0) {
    await admin.from("sige_fechamento_ads_itens").insert(
      adsManuais.map((a) => ({
        fechamento_id: fechamento.id,
        conta_ref: null,
